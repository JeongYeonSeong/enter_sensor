...
def sendData(argURL, datas):
    URL = str(argURL) + str(datas)
    print(URL)
...
def get_image(mode, image):
    
    img_ori = cv2.flip(image, -1)
    height, width, channel = img_ori.shape

#     cv2.imshow('asdfsdf1', img_ori)
# 
#     cv2.waitKey(0)
    ...
    for idx_list in result_idx:
        ...
        # (np.take(possible_contours, idx_list))print
    ...
    for r in matched_result:
        for d in r:
            images.append(temp_result[d['x'] : d['x'] + d['w'], d['y'] : d['y'] + d['h']])
            # cv2.drawContours(temp_result, d['contour'], -1, (255, 255, 255))
            cv2.rectangle(temp_result, pt1=(d['x'], d['y']), pt2=(d['x']+d['w'], d['y']+d['h']), color=(255, 255, 255), thickness=2)
    ...
    for i, matched_chars in enumerate(matched_result):
        ...
        img_cropped = cv2.getRectSubPix(
            img_rotated, 
            patchSize=(int(plate_width) + 100, int(plate_height)), 
...
#     cv2.imshow('asdfsdf10', img_result)
...
#     URL = 'http://172.26.2.69:8000/api/entryCar?numberPlate=' if mode == 1 else 'http://172.26.2.69:8000/api/exitCar?numberPlate='
# 
#     sendData(URL, chars)
...
def setServoPos(degree):
    ...
    duty = servo_MIN + (degree * (servo_MAX - servo_MIN) / 180)
